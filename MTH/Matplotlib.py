...
def var_of_means(n):
    ...
    """
    varray = np.random.normal(size=(n,n))
    meanrandom = varray.mean(axis=0)  #compute the mean of each row.
    return np.var(meanrandom)
...
def prob3():
    ...
    x2 = np.linspace(1, 6, 50, endpoint=False)
...
def prob4():
    ...
    ax1 = plt.subplot(221)
    ax1.set_title("The normal sine function")
    ax1.plot(x, np.sin(x), 'g')
    ax2 = plt.subplot(222)
    ax2.set_title("Double frequency")
    ax2.plot(x, np.sin(2*x), 'r--')
    ax3 = plt.subplot(223)
    ax3.set_title("Amplitute 2")
    ax3.plot(x, 2*np.sin(x), 'b--')
    ax4 = plt.subplot(224)
    ax4.set_title("Amplitutde 2, Double frequency")
    ax4.plot(x, 2*np.sin(2*x), 'm:')
    plt.axis([0, 2*np.pi, -2, 2])
    plt.suptitle("A family of sine functions")
    plt.show()
    return 0
...
def prob6():
    ...
    y = x.copy()
    X, Y = np.meshgrid(x, y)
    g = np.true_divide(np.sin(X)*np.sin(Y), X*Y)
    plt.subplot(121)
    plt.pcolormesh(X, Y, g, cmap="gist_earth")
    plt.colorbar()
    plt.subplot(122)
    plt.contour(X, Y, g, 15, cmap="rainbow")
    plt.colorbar()
